...
class ScaffoldGridWorldEnv(gym.Env): 
    ...
    def _check_columns_finish(self):
        ...
        difference = np.isin(difference, -ScaffoldGridWorldEnv.COL_BLOCK)

        # entry is true if we placed a scafold where colum suppose to be
        scafoldDiff = np.isin(difference,  ScaffoldGridWorldEnv.SCAFFOLD - ScaffoldGridWorldEnv.COL_BLOCK)
...
if __name__ == "__main__":
    ...
    env = ScaffoldGridWorldEnv(4, path="targets" , num_agents=1, debug=True)
